...
void kinectSkeletonAnalyzer::setup(){
    ...
    historyPlots.push_back(new ofxHistoryPlot( NULL, "angle-hand-to-hand", 100, false));
...
void kinectSkeletonAnalyzer::calculateWingspan(){
    ...
    angleHandToHand = (hands[0] - hands[1]).angle(orientation - ofVec3f(0, 1, 0));
}
...
void kinectSkeletonAnalyzer::draw(bool boundingbox){
    ...
    if(skeletons.size() > 0){
        ...
        ofPushMatrix();
        ofSetColor(ofFloatColor(ofColor::blue, ofMap(angleRightKnee, 0, 180, 0, 1, true)));
        ofTranslate(skeletons.back().pts[  skeletons.back().nameToIndex[ "KneeRight"]]);
        mesh.draw();
        ofPopMatrix();
        
        
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "KneeRight"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "AnkleRight"]]);
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "HipRight"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "KneeRight"]]);
        
        
        //Draw Angle Left Knee
        ofPushMatrix();
        ofSetColor(ofFloatColor(ofColor::blue, ofMap(angleLeftKnee, 0, 180, 0, 1, true)));
        ofTranslate(skeletons.back().pts[  skeletons.back().nameToIndex[ "KneeLeft"]]);
        mesh.draw();
        ofPopMatrix();
        
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "KneeLeft"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "AnkleLeft"]]);
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "HipLeft"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "KneeLeft"]]);
        
        //Draw Elbow Right Knee
        ofPushMatrix();
        ofSetColor(ofFloatColor(ofColor::blue, ofMap(angleRightElbow, 0, 180, 0, 1, true)));
        ofTranslate(skeletons.back().pts[  skeletons.back().nameToIndex[ "ElbowRight"]]);
        mesh.draw();
        ofPopMatrix();
        
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "HandRight"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "ElbowRight"]]);
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "ElbowRight"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "ShoulderRight"]]);
        
        //Draw Elbow Left Knee
        ofPushMatrix();
        ofSetColor(ofFloatColor(ofColor::blue, ofMap(angleLeftElbow, 0, 180, 0, 1, true)));
        ofTranslate(skeletons.back().pts[  skeletons.back().nameToIndex[ "ElbowLeft"]]);
        mesh.draw();
        ofPopMatrix();
        
        
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "HandLeft"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "ElbowLeft"]]);
        ofLine(skeletons.back().pts[  skeletons.back().nameToIndex[ "ElbowLeft"]], skeletons.back().pts[  skeletons.back().nameToIndex[ "ShoulderLeft"]]);
...
void kinectSkeletonAnalyzer::drawDebug(){
    ...
    if(skeletons.size() > 0){
        ...
        nameToHistoryPlot["angle-hand-to-hand"]->draw(200, height * count++, 190, height-5);
        count = 0;
