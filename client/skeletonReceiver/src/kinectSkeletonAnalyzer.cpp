...
void kinectSkeletonAnalyzer::calculateWingspan()
{
    ...
    for (int i = 0; i < 2; i++) {
        elbows[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "ElbowLeft" : "ElbowRight")]];
        shoulders[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "ShoulderLeft" : "ShoulderRight")]];
        hands[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "HandLeft" : "HandRight")]];
        hips[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "HipLeft" : "HipRight")]];
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        totalDist[i] = (spine - shoulders[i]).length() + (shoulders[i] - elbows[i]).length() + (elbows[i] - hands[i]).length();
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        totalHipDist[i] = (hips[i] - spineBase[i]).length() + (spineShoulder - spineBase).length() + (spineShoulder - shoulders[i]).length() + (shoulders[i] - elbows[i]).length() + (elbows[i] - hands[i]).length();
    }

    leftHandVHip = handSpineDist[0] / totalHipDist[0];
    rightHandVHip = handSpineDist[1] / totalHipDist[1];
    angleLeftElbow = (hands[0] - elbows[0])
                         .normalized()
                         .angle((elbows[0] - shoulders[0]).normalized());
    angleRightElbow = (hands[1] - elbows[1])
                          .normalized()
...
void kinectSkeletonAnalyzer::calculateStance()
{
    ...
    for (int i = 0; i < 2; i++) {
        hips[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "HipLeft" : "HipRight")]];
        knee[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "KneeLeft" : "KneeRight")]];
        feet[i] = skeletons.back().pts[skeletons.back().nameToIndex[(
            i == 0 ? "FootLeft" : "AnkleRight")]];
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        totalFeetDist[i] = (hips[i] - knee[i]).length() + (knee[i] - feet[i]).length();
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        totalFeetDist[i] = (spineBase - hips[i]).length() + (hips[i] - knee[i]).length() + (knee[i] - feet[i]).length();
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        totalFeetDist[i] = (head - spineBase).length() + (spineBase - hips[i]).length() + (hips[i] - knee[i]).length() + (knee[i] - feet[i]).length();
    }
...
{
    shouldersWidth = skeletons.back()
                         .pts[skeletons.back().nameToIndex["ShoulderLeft"] - skeletons.back().nameToIndex["ShoulderLeft"]]
                         .length();
}
