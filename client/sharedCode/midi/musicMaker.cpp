...
#define END(a) (a + (sizeof(a) / sizeof(a[0])))
	...
	shared_ptr<midiTrigger> strings = shared_ptr<midiTrigger>(new gridNote);
	...
	shared_ptr<midiTrigger> piano = shared_ptr<midiTrigger>(new gridNote);
	...
	shared_ptr<midiTrigger> accord = shared_ptr<midiTrigger>(new accordianNote);
	...
	shared_ptr<midiTrigger> legs = shared_ptr<midiTrigger>(new legCC);

	shared_ptr<midiTrigger> stompLeft = shared_ptr<midiTrigger>(new stompNote);
	...
	shared_ptr<midiTrigger> stompRight = shared_ptr<midiTrigger>(new stompNote);
	...
	shared_ptr<midiTrigger> drop = shared_ptr<midiTrigger>(new dropDatNote);
	...
	shared_ptr<midiTrigger> drumStompLeft = shared_ptr<midiTrigger>(new stompNote);
	...
	shared_ptr<midiTrigger> drumStompRight = shared_ptr<midiTrigger>(new stompNote);
...
#undef END
	...
	for(int i = 0; i < midiTriggers.size(); i++) {
		midiTriggers[i]->setMidiOut(midiOut);
	}
	for(int i = 0; i < drumMidiTriggers.size(); i++) {
		drumMidiTriggers[i]->setMidiOut(midiOut);
	}
...
Trigger musicMaker::lastPianoTriggeredNote(){
	for(int i = 0; i < midiTriggers.size(); i++) {
		if(midiTriggers[i]->_triggered){
			lastTrigger.time = midiTriggers[i]->_time;
			return lastTrigger;
		}
	}
}
			...
			if((!jazzDrums || historyAndSkeleton)){
				...
				if(!triggers[body.historyPlots[i]->varName]){
					...
					if(triggers[body.historyPlots[i]->varName]){
						...
						if(playerID == 0){
							ofNotifyEvent(triggerLinesPlayerOne, playerID, this);
						}
						if(playerID == 1){
							ofNotifyEvent(triggerLinesPlayerTwo, playerID, this);
						}
						...
						if(!ofIsStringInString(nameOfPt, "Spine")){
							if(!triggers[nameOfPt]){
								...
								if(triggers[nameOfPt]){
									...
									if(playerID == 0){
										ofNotifyEvent(triggerImplusePlayerOne, playerID, this);
									}
									if(playerID == 1){
										ofNotifyEvent(triggerImplusePlayerTwo, playerID, this);
									}
									lastTrigger.time = ofGetElapsedTimef();
								}
							}
