...
void SkeletonView::update(){
	...
	ofMatrix4x4 floor = kinect.getBodySource()->getFloorTransform();

	mat*=floor.getInverse();
	...
	for(int i = 0; i < skeletons.size(); i++){
		if(skeletons.at(i).tracked){
			tracked = true;
			KS.setFromSkeleton(skeletons.at(i), mat);
			kinectBody & body = bodyMap[skeletons.at(i).trackingId];
	...
	if(skeletons.size() > 0){
		if(stageID == 0){
			for(int i = 0; i < skeletons.size(); i++){
				...
				if(stageOne.isTriggered() && skeletons.at(i).tracked){
					//                    cout<<"Body Found"<<endl;
					return &bodyMap[skeletons.at(i).trackingId];
				}
			}
			return NULL;
		}
		if(stageID == 1){
			for(int i = 0; i < skeletons.size(); i++){
				...
				if(stageTwo.isTriggered()&& skeletons.at(i).tracked){
					//                    cout<<"Body Found"<<endl;
					return &bodyMap[skeletons.at(i).trackingId];
				}
			}
...
void SkeletonView::drawScene(ofRectangle view, ofCamera camera){
	...
	ofPushStyle();
	{
		...
		for( vector<ofxKFW2::Data::Body>::iterator iter = skeletons.begin(); iter != skeletons.end(); ++iter){
			ofPushStyle();
			bodyMap[iter->trackingId].draw();
			ofPopStyle();
		}
	}
	ofPopStyle();
...
void SkeletonView::setupGUI(){
	...
	hiddenSettings.add(scaleX.set("scaleX", 1.0,0.01, 1000));
	hiddenSettings.add(scaleY.set("scaleY", 1.0,0.01, 1000));
	hiddenSettings.add(scaleZ.set("scaleZ", 1.0,0.01, 1000));
}