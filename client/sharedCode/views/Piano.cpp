...
void Piano::update(){
	...
	if(numPlayers <= 1){
		...
		int fooButtonMode = midiView.playerOne.getInt("Output Mode");
		if(fooButtonMode != 0 || fooButtonMode != 1 || fooButtonMode != 2){
			fooButtonMode = 0;
		}

		tvView.setPlayerOneMode(fooButtonMode);
		midiView.setPlayerOneMode(fooButtonMode);
		fooButtonMode = midiView.playerTwo.getInt("Output Mode");
		if(fooButtonMode != 0 || fooButtonMode != 1 || fooButtonMode != 2){
			fooButtonMode = 0;
		}


		tvView.setPlayerTwoMode(fooButtonMode);
		midiView.setPlayerTwoMode(fooButtonMode);
